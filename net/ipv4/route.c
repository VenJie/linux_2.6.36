			  ...
			  const struct flowi *flp)
{
	...
	if (!rt_caching(net)) /*跳过路由cache，并不是所有的路由cache是有益处的*/
		goto slow_output;

	/*根据flp结构和net网络命名空间确定hash键值*/
	hash = rt_hash(flp->fl4_dst, flp->fl4_src, flp->oif, rt_genid(net)); 

	rcu_read_lock_bh(); 
	for (rth = rcu_dereference_bh(rt_hash_table[hash].chain); rth; /*查找路由cache*/
		...
		if (rth->fl.fl4_dst == flp->fl4_dst &&
		    ...
		    net_eq(dev_net(rth->dst.dev), net) && /*网络命名空间正确*/
		    !rt_is_expired(rth)) { /*路由表项未超时*/
			dst_use(&rth->dst, jiffies); 
...
slow_output:
	return ip_route_output_slow(net, rp, flp); /*路由cache未命中，查找路由表*/
}
...
/*
 * 根据flowi结构的信息，查找并存储出口路由表项
 * @net : 网络命名空间
 * @rp : 存储成功查找的路由表项
 * @flp : 查找路由表项的条件信息
 * @sk : 套接字，用于ipsec路由查找
 * @flags : 是否设置等待队列标志
 * 返回值为0表示查找成功， 非0表示查找失败
 */
